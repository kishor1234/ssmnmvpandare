...
$sql = $main->selectCount("certificate", "id"); // . $main->whereSingle(array("isActive" => "0"));
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <form action="javascript:void(0)" id="upCertificate" onsubmit="return postDataWithRedirect('<?php echo $obj->encdata("C_UploadCertificate"); ?>', '#upCertificate', '#msg', '<?php echo HOSTURL . "?r=" . $obj->encdata("C_OpenLink") . "&v=" . $obj->encdata("VCertificate"); ?>')" class="form-horizontal"   method="post" enctype="multipart/form-data">
                            <table class="table">
                                <tr>
                                    <td>
                                        <label>Certificate PDF file <span class="text-danger">*</span></label>
                                        <input type="file" required=""  id="file" name="file" placeholder="PDF Certificate" accept="application/pdf" /> 
                                    </td>

                                    <td>
                                        <label>Certificate Image file <span class="text-danger">*</span></label>
                                        <input type="file" required=""  id="imagefile" name="imagefile" placeholder="PDF Certificate" accept="image/*" /> 
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <input  class="btn btn-primary" type="submit"  value="Upload PDF" />
                                    </td>
                                    <td><br></td>
                                </tr>
                            </table>
        ...
        <div class="form-group" id='adp'>
            <div class="col-lg-12" style="overflow: auto;">
                <table id="example" class="table table-hover  table-responsive table-bordered table-striped">
                    ...
                    <?php
                    $sql = $main->select("certificate", $_SESSION["db_1"]) . $main->orderBy("DESC", "id") . $main->limitWithOffset($offset, $limit);
                    $result = $main->adminDB[$_SESSION["db_1"]]->query($sql);
                    $i = 0;
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr id="#trsk<?php echo $row["id"]; ?>">
                            ...
                            <td><a href="<?php echo $row["pdf"]; ?>" target="1"><img src="<?php echo $row["img"]; ?>" style="width:50px; height:auto;"></a></td>
                            <td><?php echo $row["ip"]; ?></td>
                            <td><?php echo $row["onCreate"]; ?></td>
                            <td>
                                <a href="javascript:void(0)" onclick="return deleteLogo('<?php echo $obj->encdata("C_DeletePdf"); ?>', '#msg', '<?php echo $row["id"]; ?>')" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a></td>
                        </tr>
                        <?php
                    }
                    ?>
