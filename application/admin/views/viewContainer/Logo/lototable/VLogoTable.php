...
$sql = $main->selectCount("logo", "id"); // . $main->whereSingle(array("isActive" => "0"));
                ...
                <div class="modal-body">
                    <center>
                        <div class="form-container">
                            <form action="/?r=C_UploadLogo" id="uploadForm" name="frmupload" method="post" enctype="multipart/form-data">
                                <div class="col-lg-6">
        ...
        <div class="form-group" id='adp'>
            <div class="col-lg-12" style="overflow: auto;">
                <table id="example" class="table table-hover  table-responsive table-bordered table-striped">
                    ...
                    <?php
                    $sql = $main->select("logo", $_SESSION["db_1"]) . $main->orderBy("DESC", "id") . $main->limitWithOffset($offset, $limit);
                    $result = $main->adminDB[$_SESSION["db_1"]]->query($sql);
                    $i = 0;
                    while ($row = $result->fetch_assoc()) {
                        $i++;
                        $dpt=array("Default","Success");
                        ?>
                    <tr id="#trsk<?php echo $row["id"]; ?>" class="<?=$dpt[$row["state"]]?>">
                            <td><?php echo $i; ?></td>
                            ...
                            <td>
                                ...
                                if ($row["state"] === "1") {
                                    ?>
                                    <a href="javascript:void(0)"  class="btn btn-success btn-sm"><i class="fa fa-save"></i> Already Set Logo</a>
                               <?php } else {
                                    ?>
                                    <a href="javascript:void(0)" onclick="return postURL('<?php echo $obj->encdata("C_SetCompanyDefaultLogo") . "&logo=" . $obj->encdata($row["logo"]) . "&l=" . $row["id"]; ?>', '#msg', '<?php echo $_SESSION["id"]; ?>')" class="btn btn-primary btn-sm"><i class="fa fa-upload"></i> Set as Default Logo</a>
                                    <?php
                                }
                                ?>
                                <a href="javascript:void(0)" onclick="return deleteLogo('<?php echo $obj->encdata("C_DeleteLogo") . "&logo=" . $obj->encdata($row["logopath"]); ?>', '#msg', '<?php echo $row["id"]; ?>')" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Delete Logo</a>
                            </td>
                        </tr>
                        <?php
                    }
